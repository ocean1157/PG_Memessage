...
- 对没有 `BUG #xxxxx` 的讨论，使用规范化主题和线程根邮件生成唯一线程键，避免同一问题在列表里散成多行。
- 本地维护状态、严重级别、标签、报错信息、复现案例代码、排查步骤和备注。
- “对话原文”会跟随官方归档的线程清单抓取关联邮件正文，并按时间线显示成类似聊天的左右气泡，清晰区分发件人、收件人、抄送、主题和附件，并自动去掉上一封邮件的引用内容；只有线程目录但没有正文的条目不会再作为空气泡显示。
...
5. 切到“对话原文”查看邮件往返；对话页只显示每封邮件新增内容，会过滤上一封邮件引用。旧数据如果只有线程目录没有正文，可以重新抓取对应月份，或选中记录后点击“补全原文”。
6. 选择“翻译为”目标语言并点击“翻译原文”，译文会直接嵌入“对话原文”的对应邮件气泡中；点击“显示原文”可切回原始内容。如需排查网页清洗结果，可以切到“纯文本原文”。
7. 在列表第一列勾选一条或多条记录后点击“删除记录”，会批量删除被勾选的本地记录；如果没有勾选，则删除当前选中记录。这不会删除 PostgreSQL 官方邮件归档。
...
没有 Bug 编号的邮件会使用规范化主题和官方线程根邮件作为唯一线程键，因此同一讨论会归并为一条记录；如果页面缺少线程信息，才回退使用 Message-ID 或归档 URL。
